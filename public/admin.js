...
/* Estado local do banner sendo editado (não persiste até "Salvar") */
let BANNER_EDIT = defaultBannerAdmin();

function ensureBannerData() {
  const base = defaultBannerAdmin();
  const raw = DATA.banner || {};
  const slots = Array.isArray(raw.slots) ? raw.slots.slice(0, 3) : [];
  while (slots.length < 3) slots.push(base.slots[slots.length]);
  return {
    enabled:   !!raw.enabled,
    titulo:    raw.titulo || base.titulo,
    subtitulo: raw.subtitulo || base.subtitulo,
...
function fillBanner() {
  BANNER_EDIT = ensureBannerData();
  $("#bn-enabled").checked = BANNER_EDIT.enabled;
  $("#bn-titulo").value = BANNER_EDIT.titulo;
  $("#bn-subtitulo").value = BANNER_EDIT.subtitulo;
  $("#bn-cta").value = BANNER_EDIT.ctaLabel;
  renderBannerSlots();
}

function bannerPerfilOptions(selected) {
  const opts = ['<option value="">— Personalizado (preencha abaixo) —</option>'];
  const perfis = [...DATA.perfis].sort((a, b) => a.nome.localeCompare(b.nome, "pt-BR"));
  perfis.forEach(p => {
    const cidade = DATA.cidades[p.cidade];
    const label = `${p.nome}${cidade ? " • " + cidade.nome : ""}`;
    opts.push(`<option value="${p.slug}" ${selected === p.slug ? "selected" : ""}>${label}</option>`);
  });
  return opts.join("");
}
...
function renderBannerSlots() {
  ...
  if (!box) return;
  box.innerHTML = BANNER_EDIT.slots.map((s, i) => `
    <div class="banner-slot" data-slot="${i}">
      <div class="banner-slot__head">
        <span class="banner-slot__num">${i + 1}</span>
        <div>
          <h4>Slot ${i + 1}</h4>
      ...
      <div class="grid2">
        ...
        <div class="field">
          ...
          <input data-bn-field="tag" value="${(s.tag || "").replace(/"/g, "&quot;")}" placeholder="Ex.: TOP 1, NOVA, EXCLUSIVA" />
        </div>
      </div>
      <div class="grid2">
        <div class="field">
          <label>Nome exibido (opcional)</label>
          <input data-bn-field="nome" value="${(s.nome || "").replace(/"/g, "&quot;")}" placeholder="Deixe vazio p/ usar o do perfil" />
        </div>
        <div class="field">
          <label>WhatsApp (opcional)</label>
          <input data-bn-field="whatsapp" value="${s.whatsapp || ""}" placeholder="55DD9XXXXXXXX" inputmode="numeric" />
        </div>
      </div>
  ...
  // bind inputs
  $$("[data-slot]", box).forEach(slotEl => {
    const i = +slotEl.dataset.slot;
    $$("[data-bn-field]", slotEl).forEach(inp => {
      inp.addEventListener("input", () => {
        const field = inp.dataset.bnField;
        let val = inp.value;
  ...
  $$("[data-bn-drop]", box).forEach(drop => {
    const i = +drop.dataset.bnDrop;
    const input = $(`[data-bn-file="${i}"]`, box);
...
$("#salvar-banner")?.addEventListener("click", async () => {
  BANNER_EDIT.enabled   = $("#bn-enabled").checked;
  BANNER_EDIT.titulo    = $("#bn-titulo").value.trim();
  BANNER_EDIT.subtitulo = $("#bn-subtitulo").value.trim();
  BANNER_EDIT.ctaLabel  = $("#bn-cta").value.trim() || "Falar no WhatsApp";

  const btn = $("#salvar-banner");
  btn.disabled = true;
  try {
    await VIPStore.saveConfig({
      adminWhatsapp: DATA.adminWhatsapp,
      metaPixelId: DATA.pixel?.metaPixelId || "",
      metaPixelEnabled: !!DATA.pixel?.metaPixelEnabled,
      banner: BANNER_EDIT,
    });
    DATA.banner = JSON.parse(JSON.stringify(BANNER_EDIT));
    toast("Banner salvo!");
  } catch (e) {
...
$("#banner-preview")?.addEventListener("click", () => {
...
async function boot() {
  try {
    ...
    DATA = { adminWhatsapp: "5515991906606", modelSupportWhatsapp: "5511996425680", pixel: { metaPixelId: "", metaPixelEnabled: false }, cidades: {}, perfis: [], stories: [] };
    updateBadge();
  }
