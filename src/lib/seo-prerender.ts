...
async function seoForHome(): Promise<SeoResult> {
  const cidades = await sbSelect<CidadeRow>("cidades", "select=slug,nome,uf&order=ordem.asc");
  if (!cidades.length) {
    return {
      status: 200,
...
async function seoForCidade(slug: string): Promise<SeoResult> {
  const [cidade] = await sbSelect<CidadeRow>(
    "cidades",
    `slug=eq.${encodeURIComponent(slug)}&select=slug,nome,uf&limit=1`,
  );
